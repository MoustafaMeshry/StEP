...
class MLPImplicitModel(nn.Module):
  def __init__(self, z_dim, num_layers=3):
    ...
    self.mlp2 = nn.Linear(z_dim, 128, bias=True)
    self.mlp3 = nn.Linear(128, 128, bias=True)
    self.mlp4 = nn.Linear(128, 128, bias=True)
    self.mlp5 = nn.Linear(128, z_dim, bias=True)
    # self.act = nn.ReLU()
...
class IMLE():
  ...
  def train(self, data_np, hyperparams, data_dir, shuffle_data=True):
    ...
    for epoch in range(hyperparams.num_epochs):
      ...
      rand_samples = self.sample(len(data_np) // 100)
...
def main(*args):
  ...
  #               base_lr=1e-2, batch_size=64, num_epochs=400, decay_step=50,  # 25
  #               decay_rate=1.0, staleness=5, num_samples_factor=10), data_dir)
  imle.train(train_data, Hyperparams(
                base_lr=1e-2,  # 1e-2, 1e-3
                batch_size=64,
                num_epochs=400,  # 1000, 400
